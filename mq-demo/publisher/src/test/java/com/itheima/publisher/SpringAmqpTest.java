...
//        rabbitTemplate.convertAndSend(exchangeName, "china.weather", msg);
        rabbitTemplate.convertAndSend(exchangeName, "gg.news", msg);
    }
        ...
        cd.getFuture().addCallback(new ListenableFutureCallback<CorrelationData.Confirm>() {
            ...
            public void onSuccess(CorrelationData.Confirm result) {
                ...
                if(result.isAck()){
                    ...
                    // 交换机故意写错，就会执行到这里
                    // 消息发送失败，可以进行消息重发
                    // 【重点：其他地方都不用管，只关注这里执行重发或者记录日志就行，有限次数重试，依然失败则记录异常消息】
                    log.error("消息发送失败，收到nack， 原因：{}", result.getReason());
                }
            }
        });

        rabbitTemplate.convertAndSend("hmall.direct111", "red21", "hello321", cd);

        // 为了看到回执，这里休眠一下，否则看不到打印
        Thread.sleep(2000);
    }
