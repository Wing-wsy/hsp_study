  ...
  rabbitmq:
    ...
    template:
      retry:
        ...
        multiplier: 1 # 失败后下次的等待时长倍数，下次等待时长 = initial-interval * multiplier 这个只是连接失败的重试，不是发送失败的重试
...
> - 交换机名称错误：同样是编程错误导致
> - MQ内部故障：这种需要处理，但概率往往较低。因此只有对消息可靠性要求非常高的业务才需要开启，并且无需开启Publisher-Return机制，仅仅需要开启ConfirmCallback处理nack就可以了，对于nack消息可以有限次数重试，依然失败则记录异常消息。
...
而在**3.12版本之后，LazyQueue已经成为所有队列的默认格式（无法更改）**。因此官方推荐升级MQ为3.12版本或者所有队列都设置为LazyQueue模式。
...
### 4.5.3.改造下单业务【一定频率重试调用，业务中有这样的场景可以参考这个实现方案】

【视频：https://www.bilibili.com/video/BV1mN4y1Z7t9?p=31&spm_id_from=pageDriver&vd_source=dc02a4c6e2a8e915fb8ee431999e5b2c】
